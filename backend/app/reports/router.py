...
async def generate(req: GenerateReportRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        report = await generate_report(db=db, report_type=req.report_type, asset_id=req.asset_id, anomaly_event_id=req.anomaly_event_id)
...
async def list_reports(
    ...
    result = await db.execute(
        select(AnalysisReport).where(where).order_by(AnalysisReport.created_at.desc()).limit(limit).offset(offset)
    )
