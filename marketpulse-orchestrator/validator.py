    ...
    for cf in changed:
        ...
        for exp in files_expected:
            ...
            if cf == exp or cf.startswith(exp.rstrip("/") + "/") or exp.startswith(cf.rstrip("/") + "/"):
                ...
                break
        if not matched:
            # Allow common infrastructure files that agents may touch
            basename = os.path.basename(cf)
            if basename in ("__init__.py", ".gitkeep") or "__pycache__" in cf or cf == "rationale.md":
                continue
            # Allow orchestrator task_store, reports, and logs (generated by agent system)
            if cf.startswith("marketpulse-orchestrator/task_store/") or cf.startswith("reports/") or cf.startswith("logs/"):
                continue
