...
- **Status:** `pass`
- **Evidence:** `mypy app/recommendations/scoring.py --ignore-missing-imports` → Success: no issues found in 1 source file
...
- **Status:** `pass`
- **Evidence:** test_weights_sum_to_one passes — sum(WEIGHTS.values()) == 1.00
...
- **Status:** `pass`
- **Evidence:** `pytest tests/ -q` → 234 passed in 1.25s
